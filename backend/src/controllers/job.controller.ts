    ...
    async getAllJobs(req: Request, res: Response) {
        try {
            ...
            if (req.query.jobType) {
                const parsed = JobTypeEnum.safeParse(req.query.jobType);
                if (!parsed.success) {
            ...
            if (req.query.workMode) {
                const parsed = WorkModeEnum.safeParse(req.query.workMode);
                if (!parsed.success) {
                    return res.status(400).json({ success: false, message: "Invalid workMode filter" });
                }
                workMode = parsed.data;
            }

            let experienceLevel: ExperienceLevelType | undefined;
            if (req.query.experienceLevel) {
                const parsed = ExperienceLevelEnum.safeParse(req.query.experienceLevel);
                if (!parsed.success) {
